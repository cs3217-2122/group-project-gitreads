        ...
        ScrollView(.horizontal) {
            HStack(spacing: 0) {
                ForEach((0..<line.tokens.count), id: \.self) { pos in
                    TokenView(viewModel: viewModel, token: line.tokens[pos], lineNum: lineNum, pos: pos, fontSize: $fontSize)
                }
            }
        }
...
struct ScrollLineView_Previews: PreviewProvider {
    ...
    static var previews: some View {
        ScrollLineView(viewModel: ScreenViewModel(), line: Line(tokens: [Token(type: .keyword, value: "1TESgfdgdfgsdfgdfsgdfgdsfgdsfgfgT"),
                                           ...
                                           Token(type: .keyword, value: "14TEST")]), lineNum: 1, fontSize: $fontSize)
    }
}
